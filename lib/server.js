...
class GoFarmTechServer extends EventEmitter {
	...
	initialize() {
		...
		this.server.on('connection', stream => {
			...
			client.on('connect', options => {
				...
				if (this.config.user) {
					if(this.config.user !== PwdProtector.encrypt(options.username + "") || this.config.pass !== PwdProtector.encrypt(options.password + "")) {
	...
	publishToClient(id, state, withoutEvent) {
		let deviceId = this.deviceIdFromStateId(id);
		let doEmitOffline = (e) => {
			if(!withoutEvent) {
				setTimeout(() => {
					this.emit('offlineState', deviceId, id, state);
				}, 10);
			}
			this.error('PublishToClient Error', e);
		};
		...
		if(deviceId in this.clients) {
			...
			return client.publishPayload(makePayload(topic, val)).catch(doEmitOffline);
		} else {
			return Promise.reject(new Error('Client not exists')).catch(doEmitOffline);
		}
	}
}
